...
class Tester(object):
    ...
    def configure(self, task_name, private_config_path):
        """
        ...
        self.private_config = yaml.load(fs, Loader=yaml.FullLoader)
        self.run_file = self.private_config["BACKUP_CONFIG"]["main_file"]
    ...
    def __copy_source_code(self, run_file, code_dir):
        ...
        if self.private_config["PROJECT_TYPE"]["backup_code_by"] == 'lib':
            ...
            shutil.copytree("../../../build/lib/", code_dir)
            ...
            for dir_name in self.private_config["BACKUP_CONFIG"]["backup_code_dir"]:
                shutil.copytree("../../" + dir_name, code_dir + '/' + dir_name)
        else:
            raise NotImplementedError
    ...
    def save_checkpoint(self):
        import tensorflow as tf
        self.saver.save(tf.get_default_session(), self.checkpoint_dir + 'checkpoint', global_step=self.time_step_holder.get_time())
