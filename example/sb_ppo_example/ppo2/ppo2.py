...
class PPO2(ActorCriticRLModel):
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, update,
        ...
        if self.full_tensorboard_log and (1 + update) % 10 == 0:
            ...
            summary, policy_loss, value_loss, policy_entropy, approxkl, clipfrac, _ = self.sess.run(
                ...
                td_map)
        # [RLA] phase 3: log
        time_step_holder.set_time(update * update_fac)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2", reset_num_timesteps=True):
        ...
        # [RLA] (optional) construct a saver.
        with self.graph.as_default():
        ...
        for update in range(1, n_updates + 1):
            ...
            if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                ...
                logger.dumpkvs()
            # [RLA] (optional) add a checkpoint
            if update % 100 == 0:
                params_to_save = self.get_parameters()
                with self.sess.as_default():
                    exp_manager.save_checkpoint(related_variable=params_to_save)
